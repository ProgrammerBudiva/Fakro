...
class ControllerCheckoutConfirm1 extends Controller {
    public function index() {
        ...
        if(empty($this->request->post['payment_method'])) {
            $json['error'][] = 'Не указан вид оплаты!';
        }
        ...
        if (!$redirect && empty($json)) {
            $order_data = array();


            switch ($this->request->post['payment_method']){
                case 1: $order_data['payment_method'] = 'Предоплата через Приват24';break;
                case 2: $order_data['payment_method'] = 'Наложенный платеж (доп. +2% услуги Новой Почты)';break;
                case 3: $order_data['payment_method'] = 'Оплата по счёту с НДС (безналичный расчет)';break;
            }
